...
Each row in the Task 1 results file should be formatted as follows:
<id> <TAB> <class_label> <TAB> <run_id>

where:
<id> is the ID of the tweet/sentence
<class_label> indicates the predicted label of the given tweet.
...
def check_format(file_path):
    logging.info(f"Task 1: Checking format of file: {file_path}")
    with open(file_path, encoding='UTF-8') as out:
        next(out)
        file_content = out.read().strip()
        for i, line in enumerate(file_content.split('\n')):
            if len(line.strip().split('\t')) < 3:
                logging.error(f"Missing columns in line: {line}")
                return False
    ...
    logging.info(f"Task 1: No issue found.")
    return True
